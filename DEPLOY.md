...
Mọi checkpoint trong `checkpoints/` được train giả định drivetrain đạt gần
những số này (`javis/sim_config.py`, `DrivetrainCfg`). Nếu board thật chỉ lên
được gain thấp hơn, **đo lại số thật rồi kiểm tra bằng**:

```bash
.venv/bin/python scripts/eval_payload_sweep.py --checkpoint checkpoints/javis_payload_flat/model_1499.pt \
...
Nguồn chân lý là `checkpoints/javis_payload_flat/model_1499_contract.json`
(sinh tự động bởi `scripts/export_onnx.py`, không gõ tay — luôn export lại
file này mỗi khi có checkpoint mới, đừng copy số cũ).

### Observation — 192 giá trị, **TERM-MAJOR**, không phải frame-major

Đây là chỗ dễ sai nhất theo đúng cảnh báo PLAN.md đã nêu. Toàn bộ vector KHÔNG
phải là 12 khung hình liên tiếp mỗi khung 16 giá trị. Nó là:

```
[ base_lin_vel: 12 khung cũ→mới ][ base_ang_vel: 12 khung ][ projected_gravity: 12 khung ]
[ wheel_vel: 12 khung ][ actions: 12 khung ][ command: 12 khung ]
```

| Term | Rộng | Nguồn thật trên robot | Đã có sẵn? |
|---|---|---|---|
| `base_lin_vel` | 12×3 | **KHÔNG phải IMU** — xem mục sửa lỗi bên dưới. Cần odometry từ `orin_vslam_bringup` (cuVSLAM) | ⚠️ Pipeline có sẵn nhưng chưa nối vào `policy_inference_node` |
| `base_ang_vel` | 12×3 | `/bmx160_bmp388_node/imu/data_raw`, trường `angular_velocity` | ✅ có sẵn |
| `projected_gravity` | 12×3 | Cần **orientation đã fusion** (`imu_filter_madgwick`), không lấy thẳng từ IMU thô | ⚠️ Launch file có điều kiện, cần bật (PLAN.md mục 3.2) |
| `wheel_vel` | 12×2 | `/wheel_states` (`javis_odrive_driver` publish, **rad/s**, đổi từ `vel_estimate` turn/s: `×2π`) | ❌ driver chưa viết |
| `actions` | 12×2 | Policy tự lưu lại action bước trước, không qua ROS2 topic | — (nằm trong `policy_inference_node`) |
| `command` | 12×3 | `/cmd_vel`-kiểu twist, nguồn chưa quyết (PLAN.md mục 3.3, đề xuất teleop trước) | ❌ chưa có nguồn |

Mỗi term còn có noise huấn luyện + trễ ngẫu nhiên 1-3 bước — driver thật
**không cần** giả lập trễ (trễ thật vốn đã có), chỉ cần đưa đúng số đo mới
nhất mỗi tick.
...
### Tần số điều khiển: 50 Hz — không đổi, dù timestep vật lý đã đổi

Task giờ chạy physics ở 2.5 ms/400 Hz thay vì 5 ms/200 Hz (lý do: vòng PI cần
timestep mịn hơn để ổn định ở gain cao, xem `SIM2REAL.md` mục 3), nhưng
`decimation` cũng tăng gấp đôi (4→8) để giữ nguyên **50 Hz điều khiển** —
không cần đổi gì ở phía `policy_inference_node`. Vẫn cần benchmark số thật
trên Jetson (PLAN.md mục 7) — 50 Hz vẫn là giả định, không phải số đo.
...
| Việc | Lệnh |
|---|---|
| Lái thử policy bằng bàn phím trong sim, trước khi đụng phần cứng | `scripts/teleop_keyboard.py --checkpoint checkpoints/javis_payload_flat/model_1499.pt` (W/S/A/D, xem `--help`) |
...
3. Benchmark tần số/độ trễ USB thật (PLAN.md mục 7). Nếu lệch đáng kể khỏi
   50 Hz, sửa `decimation` trong `javis/balance_task.py` rồi **train lại** —
   đừng chỉnh ngược ở phía driver để "giả vờ" khớp 50Hz.
...
5. Viết `policy_inference_node`: nạp `checkpoints/javis_payload_flat/model_1499.onnx`,
   lắp buffer lịch sử **đúng thứ tự term-major**, publish theo bảng action ở
   trên. Test lại bằng `scripts/export_onnx.py`'s parity check nếu nghi ngờ.
...
- `checkpoints/javis_payload_flat/model_1499.pt` là checkpoint smoke-test
  (1500 iteration, curriculum đạt 0.96/1.0), **chưa hội tụ**, chưa từng chạy
  địa hình gồ ghề/dốc — chỉ `Javis-Payload-Flat`. Xem `checkpoints/README.md`.
...
- Độ trễ USB thật, tần số control loop thật: chưa đo (PLAN.md mục 7).
